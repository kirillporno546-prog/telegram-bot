...
async def buy_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ИСПРАВЛЕНО: buy:gift:price — разбиваем максимум на 3 части (имя подарка может содержать ':')
    ...
    # ИСПРАВЛЕНО: проверяем нет ли уже pending-заявки у пользователя (защита от спама)
    cur = get_cur()
...
async def lottery_buy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ИСПРАВЛЕНО: покупка и проверка лимита в одной транзакции чтобы избежать гонки
    cur = get_cur()
    ...
    try:
        ...
        if current_count >= limit:
            ...
            await query.answer("❌ Лотерея уже заполнена, ожидайте следующей.", show_alert=True)
            return
