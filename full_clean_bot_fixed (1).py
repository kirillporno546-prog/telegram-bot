...
async def task_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    ensure_user(uid, username)  # гарантируем существование записи в users перед любыми UPDATE
    ...
    cur.execute(
        "UPDATE users SET balance = COALESCE(balance, 0) + ?, tasks_completed = COALESCE(tasks_completed, 0) + 1 WHERE user_id=?",
        (float(reward), uid),
    )
